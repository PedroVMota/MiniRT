...
bool	generate_data(void)
{
	...
	if (!ft_strcmp((scene())->objd[0], "C"))
		object = create_camera((scene())->objd, CAMERA);
	else if (!ft_strcmp((scene())->objd[0], "L"))
		object = create_light((scene())->objd, POINT);
	else if (!ft_strcmp((scene())->objd[0], "A"))
		object = create_light((scene())->objd, AMBIENT);
	else if (!ft_strcmp((scene())->objd[0], "pl"))
		object = generate_pl((scene())->objd);
	else if (!ft_strcmp((scene())->objd[0], "sp"))
		object = create_sp((scene())->objd);
	else if (!ft_strcmp((scene())->objd[0], "cy"))
		object = create_cy((scene())->objd, CYLINDER);
	if (object != NULL)
